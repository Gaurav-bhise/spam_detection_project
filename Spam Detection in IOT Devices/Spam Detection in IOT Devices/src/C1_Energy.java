...
public class C1_Energy {
	public static void main(String[] args) {
		...
		try {
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/proj20","root","");System.out.println("Connected");
			Statement st = con.createStatement();

			ResultSet rs = st.executeQuery("select * from cluster1");
			...
			JFreeChart chart = ChartFactory
					.createBarChart(
							"Spam Detection Techniues for IOT Devices --- Cluster1",
							"Node Category--Cluster1", "Energy", dataSet,
							PlotOrientation.VERTICAL, true, true, true);

			ChartFrame chartFrame = new ChartFrame(
					"Cluster1 Different Transaction Energy Details", chart);
